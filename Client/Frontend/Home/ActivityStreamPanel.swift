...
extension ActivityStreamPanel {
    ...
    private func presentContextMenu(site: Site, indexPath: NSIndexPath) {
        ...
        let dismissAction = ActionOverlayTableViewAction(title: Strings.DismissContextMenuTitle, iconString: "action_close", handler: { action in
            self.profile.recommendations.removeHighlightForURL(site.url)
                .upon { res in
                    self.fetchHighlights().uponQueue(dispatch_get_main_queue()) { result in
                        if let history = result.successValue {
                            self.history = history.asArray()
